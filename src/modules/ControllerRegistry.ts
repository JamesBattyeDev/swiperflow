...
class ControllerRegistry {
  ...
  getConnections() {
    let allThumbs = Array.from(document.querySelectorAll(`[${config.keyWithPrefix('thumb-id')}]`)) as HTMLElement[]
    allThumbs.forEach((thumb, thumbIndex) => {
      ...
      allThumbs.forEach((comparisonThumb, compIndex) => {
        ...
        if (thumb.dataset.swfThumbId === comparisonThumb.dataset.swfThumbId) {
          ...
          this.#connections.set((thumb.parentNode as SwiperElement).swiper!, (comparisonThumb.parentNode as SwiperElement).swiper!)
        }
      })
